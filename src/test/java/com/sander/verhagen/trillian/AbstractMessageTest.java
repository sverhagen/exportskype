...
/**
 * Tests for {@link AbstractMessage} and as thus serving as an abstract base
 * class for tests for children of {@link AbstractMessage}.
 * 
...
public abstract class AbstractMessageTest {
	...
	/**
	 * Prepare some expectations. Would've been a constant, but need to call
	 * <code>getType()</code>
	 */
	@Before
	public void setUp() {
	...
	abstract protected AbstractMessage getMessage(Chat chat, Message message,
			String to);
	...
	private Message createMockMessage(boolean incoming) {
		...
		expect(message.getAuthor()).andReturn(incoming ? OTHER : HOME_USER)
				.anyTimes();
